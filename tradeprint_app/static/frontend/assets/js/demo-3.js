...
(function ($) {
    ...
    /*----------------------------- Main Slider ---------------------*/
    // Duplicate initialization removed used to be here
    /* $('.ec-slider').slick({
        rows: 1,
        dots: true,
        arrows: true,
    ...
    window.addEventListener("scroll", function () {
        const header = document.getElementById("ec-main-menu-desk");
        if (window.scrollY > 100) { // adjust 100 if you want it earlier/later
    ...
    // Initialize Slick
    $('.ec-slider').slick({
        dots: false, // we are using custom dots
        arrows: true,
        infinite: true,
        speed: 600,
        slidesToShow: 1,
    ...
    // When slide changes → update active dot
    $('.ec-slider').on('afterChange', function (event, slick, currentSlide) {
        currentIndex = currentSlide;
        $('.ec-slider-dots .dot').removeClass('active');
        $('.ec-slider-dots .dot[data-slide="' + currentSlide + '"]').addClass('active');
    });

    // When clicking on custom dots
    $('.ec-slider-dots').on('click', '.dot', function () {
        const clickedIndex = $(this).data('slide');

        // ✅ Determine direction (like your get_prev_slick_img / get_next_slick_img idea)
        if (clickedIndex < currentIndex) {
            // previous slide
            $('.ec-slider').slick('slickPrev');
        } else if (clickedIndex > currentIndex) {
            // next slide
            $('.ec-slider').slick('slickNext');
        } else {
            // same slide, no change
            return;
        }
    });
    ...
    $(document).ready(function () {
        ...
        if ($('#ec-testimonial-slider').length) {
            $('#ec-testimonial-slider').slick({
                slidesToShow: 1,
                slidesToScroll: 1,
                dots: true,                // show dots automatically
                ...
                cssEase: 'linear',
                adaptiveHeight: true,
                appendDots: $('.ec-test-section .ec-slider-dots') // append to the testimonial dots container
            });

            // Custom dot styling with images (optional enhancement)
            setTimeout(function () {
...
vatSwitch.addEventListener("click", () => {
    vatSwitch.classList.toggle("active");
});

/*------------------slider dots ----------------------*/
const slides = document.querySelectorAll('.ec-slide-item');
const dots = document.querySelectorAll('.ec-slider-dots .dot');
let currentSlide = 0;
let slideInterval;
...
function startAutoSlide() {
    slideInterval = setInterval(nextSlide, 1000); // changes every 3 seconds
}

dots.forEach((dot, index) => {
    dot.addEventListener('click', () => {
        clearInterval(slideInterval); // stop auto when manually clicked
        currentSlide = index;
        showSlide(index);
